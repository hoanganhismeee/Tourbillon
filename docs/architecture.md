...
- `ChatService` — Chat concierge orchestration. Short-circuits abusive and unrelated prompts, resolves exact watches, ordinal follow-ups, compare requests, and cursor commands deterministically, rewrites discovery actions into canonical catalogue queries, and sends only compact Tourbillon catalogue context to ai-service when explanation helps. Signed-in users also send a compact Watch DNA + behavior summary so responses can adapt to their inferred taste without exposing raw profile internals. The frontend also sends an explicit response-language hint, which the backend normalizes before forwarding to ai-service. Redis session store now carries history, last surfaced cards, compare scope, and follow-up mode with a 1-hour TTL. Rate-limited per user/day. System prompt is scoped, grounded, anti-hallucination, and catalogue-bound by default. Limited web enrichment is allowed only for approved brand-history questions, and those web notes are treated as secondary context that cannot override catalogue facts or create shopping actions.
